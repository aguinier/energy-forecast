...
def replica(tmp_path, monkeypatch):
    ...
    for ts in pd.date_range(OBS - pd.Timedelta(days=1), OBS + pd.Timedelta(days=3), freq="h"):
        ...
        con.execute(
            "INSERT INTO weather_data VALUES (?, ?, ?, 'forecast', ?, ?, ?)",
            (COUNTRY, str(ts), str(late_run), POISON, POISON, POISON),
        )
    con.commit()
    con.close()
...
def test_supported_forecast_types_is_wind_only():
    """ABL-183 scope: only the two types ABL-179 diagnosed. See
    wind_features.py's SUPPORTED_FORECAST_TYPES docstring for why
    solar/hydro_total/biomass/renewable are not included yet."""
    assert set(SUPPORTED_FORECAST_TYPES) == {"wind_onshore", "wind_offshore"}


def test_builder_refuses_an_unsupported_forecast_type(replica):
    with pytest.raises(ValueError):
        RenewableFeatureBuilder(COUNTRY, "solar", OBS - pd.Timedelta(days=1), OBS)
